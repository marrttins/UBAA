...
@section('content')
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 mb-6 md:mb-10">
    <!-- Stat Card: Users -->
    <div class="bg-white rounded-[20px] md:rounded-[24px] p-6 md:p-8 shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative overflow-hidden group">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-2xl md:text-3xl font-extrabold text-gray-800">{{ $stats['users'] }}</h3>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[20px] md:rounded-[24px] p-6 md:p-8 shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative overflow-hidden group">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-2xl md:text-3xl font-extrabold text-gray-800">{{ $stats['jobs'] }}</h3>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[20px] md:rounded-[24px] p-6 md:p-8 shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative overflow-hidden group">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-2xl md:text-3xl font-extrabold text-gray-800">{{ $stats['events'] }}</h3>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[20px] md:rounded-[24px] p-6 md:p-8 shadow-sm border border-gray-100 hover:shadow-md transition-shadow relative overflow-hidden group">
        ...
        <div class="relative z-10">
            ...
            <h3 class="text-2xl md:text-3xl font-extrabold text-gray-800">{{ $stats['news'] }}</h3>
        </div>
    </div>
</div>

<div class="bg-white rounded-[24px] md:rounded-[32px] p-6 md:p-10 border border-gray-100 shadow-sm relative overflow-hidden">
    ...
    <div class="relative z-10 max-w-2xl">
        ...
        <h3 class="text-2xl md:text-4xl font-extrabold text-gray-900 mb-6 leading-tight">Welcome to the Command Center, <span class="text-[var(--primary)]">{{ auth()->user()->first_name ?? auth()->user()->name }}</span>.</h3>
        <p class="text-gray-500 text-base md:text-lg leading-relaxed mb-8 font-medium">
        ...
        <div class="flex flex-wrap gap-4">
            <a href="{{ route('admin.users') }}" class="bg-[var(--primary)] text-white px-6 md:px-8 py-3 md:py-4 rounded-2xl font-bold hover:bg-[var(--primary-dark)] transition-all shadow-lg shadow-purple-200 text-sm md:text-base">Manage Members</a>
            <a href="{{ route('admin.news') }}" class="bg-white text-[var(--primary)] border-2 border-[var(--primary)] px-6 md:px-8 py-3 md:py-4 rounded-2xl font-bold hover:bg-purple-50 transition-all text-sm md:text-base">Latest Updates</a>
        </div>
    </div>
</div>
@endsection
